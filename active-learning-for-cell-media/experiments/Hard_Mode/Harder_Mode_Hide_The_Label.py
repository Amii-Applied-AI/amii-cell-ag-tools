...
MODE_TAG = 'Hard_Mode_Hide_The_Label'
...
class HarderHideLabelCompetition(HideLabelCompetitionIncrementalGP):
    ...
    def _generate_harder_model_cache_key(self, X: np.ndarray, y: np.ndarray, dataset_name: Optional[str]) -> str:
        """Create a unique cache key for harder surrogate based on data, config, and experiment mode."""
        ...
        h.update(f"{MODE_TAG}_{dataset_name}_{self.surrogate_type}_{self.random_state}".encode())
        ...
        return f"{MODE_TAG}_{h.hexdigest()[:16]}"
    ...
    def fit_surrogate_model(self, X: np.ndarray, y: np.ndarray, y_var: np.ndarray, dataset_name: str = None, use_cache_model: bool = True) -> Tuple[Any, StandardScaler]:
        ...
        elif self.surrogate_type == 'bayesian_neural_network':
            surrogate = HarderSurrogateFactory.create_harder_surrogate(
                surrogate_type='bayesian_neural_network',
                hidden_sizes=[128, 64],
                dropout_rate=0.2,
                n_mc_samples=50,
                learning_rate=1e-3,
...
def run_harder_competition(dataset_name: str, 
                          ...
                          pregenerated_synthetic_datasets: list = None,
                          competition_start: int = 0) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """
    ...
    surrogate_model, scaler = competition.fit_surrogate_model(X, y, y_var, dataset_name, use_cache_model=use_cache_model)
    ...
    # Generate all synthetic datasets at once (or reuse pre-generated ones)
    if pregenerated_synthetic_datasets is not None:
        synthetic_datasets = pregenerated_synthetic_datasets
        print(f"\n=== Reusing {len(synthetic_datasets)} Pre-generated Synthetic Datasets ===")
    else:
    ...
    for synth_idx, (X_pool, y_pool) in enumerate(synthetic_datasets):
        ...
        # Sparse progress to fd 1 (bypasses _silence redirection in topup workers)
        if synth_idx % 5 == 0 or synth_idx == num_of_synthetic_data - 1:
            try:
                import os as _os
                _os.write(1, f"  [synth {synth_idx+1}/{num_of_synthetic_data}] starting {n_competitions} comps (B={batch_size}, hf={hidden_fraction})\n".encode())
            except Exception:
                pass
        _t_synth = time.time()
        # Run tournament
        ...
        if synth_idx % 5 == 0 or synth_idx == num_of_synthetic_data - 1:
            try:
                _os.write(1, f"  [synth {synth_idx+1}/{num_of_synthetic_data}] done in {time.time()-_t_synth:.1f}s\n".encode())
            except Exception:
                pass
    ...
    if num_of_synthetic_data > 1:
        ...
        # Build combined results dict (always, regardless of save_results)
        combined_results = {
            'dataset_name': dataset_name,
            'surrogate_type': surrogate_type,
            'difficulty_config': difficulty_config,
