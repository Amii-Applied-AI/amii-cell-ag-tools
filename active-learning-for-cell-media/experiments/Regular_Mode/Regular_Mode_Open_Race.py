...
from experiments.Visualization_and_Tables.Open_Race_best_so_far_plots import (
    load_open_race,
...
def open_race_game(f: Callable[[np.ndarray], float], B: int, optimizers: List[OptimizationStrategy], 
    ...
    # Run all optimizers IN PARALLEL (each is fully independent with its own D copy)
    from concurrent.futures import ThreadPoolExecutor

    def _run_one_or_optimizer(optimizer, D_init_X, D_init_y, f, S, R, B, bounds):
        """Run a single optimizer through its full open-race trajectory."""
        optimizer.reset()
        D_X = D_init_X.copy()
        D_y = D_init_y.copy()
        current_best = np.max(D_y)

        optimization_history = [{'step': 0, 'evaluations': R,
        ...
        for s in range(n_steps):
            current_batch_size = remaining_evaluations if (s == n_steps - 1 and remaining_evaluations > 0) else B

            try:
                X_next = optimizer.select_next_batch_open_field(
                    X_observed=D_X, y_observed=D_y, current_best=current_best,
                    batch_size=current_batch_size, bounds=bounds, true_function=f)
                if X_next.ndim == 1:
                ...
                optimization_history.append({
                    'step': s + 1, 'evaluations': R + (s + 1) * current_batch_size,
                    'best_value_so_far': float(current_best), 'current_best': float(current_best),
                    'batch_size': current_batch_size, 'new_values': Y_next.tolist()})
        ...
        return optimizer.name, best_value, optimization_history

    observation_list = []
    optimization_histories = {}

    with ThreadPoolExecutor(max_workers=len(continuous_optimizers)) as executor:
        futures = [
            executor.submit(_run_one_or_optimizer, opt, D_init_X, D_init_y, f, S, R, B, bounds)
            for opt in continuous_optimizers
        ]
        # Collect results preserving original order
        results_map = {}
        for future in futures:
            name, best_val, history = future.result()
            results_map[name] = (best_val, history)

    # Preserve original optimizer order in observation_list
    for opt in continuous_optimizers:
        best_val, history = results_map[opt.name]
        observation_list.append(best_val)
        optimization_histories[opt.name] = history
...
class OpenRaceCompetition:
    ...
    def __init__(self, random_state: int = 42):
        ...
        # Cache directory for Regular Mode Open Race models
        self.cache_dir = str(ADDITIONAL_OUTPUTS_DIR / "regular_open_race_model_cache")
    ...
    def _generate_or_cache_key(self, X: np.ndarray, y: np.ndarray, dataset_name: str) -> str:
        """Generate unique cache key for Regular Mode Open Race surrogate models"""
        h = hashlib.md5()
        h.update(f"{self.MODE_TAG}_{dataset_name}_{self.random_state}".encode())
        ...
        return f"{self.MODE_TAG}_{h.hexdigest()[:16]}"
    
    def _load_or_cached_model(self, cache_key: str) -> Optional[Tuple[GaussianProcessRegressor, StandardScaler, float]]:
        """Load cached GP model if available"""
        candidates = []
        try:
        ...
        try:
            logger.info(f"Loading Regular Open Race cached model from: {chosen}")
            payload = load(chosen)
            if isinstance(payload, tuple) and len(payload) >= 3:
                gp, scaler, r2 = payload[:3]
                logger.info(f"Successfully loaded cached GP model with R²: {float(r2):.4f}")
                return gp, scaler, float(r2)
        except Exception as e:
            logger.warning(f"Failed to load Regular Open Race cached model: {e}")
        
        return None
    
    def _save_or_cached_model(self, cache_key: str, gp: GaussianProcessRegressor, scaler: StandardScaler, r2: float, dataset_name: str) -> None:
        """Save GP model to cache"""
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        safe_dataset = str(dataset_name).replace(' ', '_')
        filename = f"{cache_key}__{self.MODE_TAG}__{safe_dataset}__{timestamp}.joblib"
        path = os.path.join(self.cache_dir, filename)
        try:
            dump((gp, scaler, float(r2)), path)
            logger.info(f"Saved Regular Open Race model to cache: {path}")
        except Exception as e:
            logger.warning(f"Failed to save Regular Open Race cached model: {e}")
    ...
    def fit_surrogate_model(self, X: np.ndarray, y: np.ndarray, y_var: np.ndarray, 
                           dataset_name: str = None, use_cache_model: bool = True) -> Tuple[GaussianProcessRegressor, StandardScaler]:
        ...
        cache_key = self._generate_or_cache_key(X, y, dataset_name or "unknown")
        logger.info(f"Regular Open Race cache key: {cache_key}")
        
        # Try to load from cache first
        if use_cache_model:
            cached_result = self._load_or_cached_model(cache_key)
            if cached_result is not None:
                gp, scaler, r2 = cached_result
                self._log_timing("Loaded cached GP surrogate model")
                return gp, scaler
        
        # For T/TF datasets, also try to reuse Hide-The-Label cache as fallback
        use_cached_htl = use_cache_model and dataset_name in ['df_Human_T_Cell_Expanded', 'df_Human_TF_Cell_Expanded']
        ...
        if use_cached_htl:
            ...
            try:
                ...
                gp, scaler, r2 = htl.fit_surrogate_model(
                    X, y, y_var, dataset_name=dataset_name, use_cache_model=use_cache_model
                )
                ...
                # Also save to Open Race cache for future runs
                if use_cache_model:
                    self._save_or_cached_model(cache_key, gp, scaler, r2, dataset_name)
                return gp, scaler
            except Exception as e:
        ...
        # Save to cache
        if use_cache_model:
            self._save_or_cached_model(cache_key, gp, scaler, r2, dataset_name or "unknown")
    ...
    def run_competition_tournament(self, dataset_name: str, optimizer_names: List[str] = None,
                                  ...
                                  noise_std: float = 0.0, n_jobs: int = -1, use_cache_model: bool = True) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            use_cache_model: Whether to use cached surrogate models if available
...
def run_open_race_competition(dataset_name: str, 
                             ...
                             use_cache_model: bool = True) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """
