...
# Module-level optimizer step caps — set by topup workers to limit random-walk optimizers.
# Keys are optimizer names, values are max steps (e.g. {'RANDOM': 60, 'DE_DIRECT': 60}).
_OPTIMIZER_MAX_STEPS = {}
...
from experiments.Visualization_and_Tables.Hide_The_Label_bar_plots import (
...
class HideLabelCompetitionIncrementalGP:
    ...
    def _generate_model_cache_key(self, X: np.ndarray, y: np.ndarray, dataset_name: str) -> str:
        """
        Generate a unique cache key for the model based on data, dataset name, and experiment mode.
        ...
        # IMPORTANT: Include MODE_TAG to ensure cache separation between experiment modes
        data_hash = hashlib.md5()
        data_hash.update(f"{MODE_TAG}_{X.shape}_{y.shape}_{dataset_name}_{self.random_state}".encode())
        data_hash.update(X.flatten()[:100].tobytes())  # First 100 values
        data_hash.update(y.flatten()[:100].tobytes())  # First 100 values
        
        return f"{MODE_TAG}_{data_hash.hexdigest()[:16]}"
    ...
    def run_single_competition(self, X_pool: np.ndarray, y_pool: np.ndarray,
        ...
        # Run each optimizer IN PARALLEL (optimizers are independent — each gets its own copy of state)
        optimizer_results = {}

        # IMPORTANT: Clear GP cache once before all optimizers in this competition
        from utils.optimizers import _GP_CACHE
        _GP_CACHE.clear()

        # Per-optimizer step caps (avoid slow random-walk optimizers dominating wall-clock)
        # Read from env var (reliable across spawned processes) or module-level or instance attr
        _optimizer_max_steps = {}
        _env_cap = os.environ.get('OPTIMIZER_MAX_STEPS_JSON')
        if _env_cap:
            import json as _json
            _optimizer_max_steps = _json.loads(_env_cap)
        if not _optimizer_max_steps:
            _optimizer_max_steps = _OPTIMIZER_MAX_STEPS or getattr(self, 'optimizer_max_steps', None) or {}

        def _run_one_optimizer(optimizer_name, optimizer, competition_state, X_pool, y_pool,
                               target_idx, target_value, batch_size, local_rng_seed, verbose,
                               max_steps=None):
            ...
            opt_rng = np.random.RandomState(local_rng_seed)
            ...
            while not found_target and len(hidden_indices) > 0 and (max_steps is None or steps_to_target < max_steps):
                ...
                try:
                    if batch_size == 1:
                        next_indices = [optimizer.select_next_point(
                            X_pool=X_pool, hidden_indices=hidden_indices,
                        ...
                        next_indices = optimizer.select_next_batch(
                            X_pool=X_pool, hidden_indices=hidden_indices,
                            X_observed=X_observed, y_observed=y_observed,
                            current_best=current_best_local, batch_size=actual_batch_size)
                except Exception:
                    next_indices = opt_rng.choice(hidden_indices, size=actual_batch_size, replace=False).tolist()

                # Validate and sanitize selections
                ...
                for idx in next_indices:
                    if idx not in seen: deduped.append(idx); seen.add(idx)
                ...
                if needed > 0:
                    ...
                    if len(remaining_choices) >= needed:
                        proposed.extend(opt_rng.choice(remaining_choices, size=needed, replace=False).tolist())
                    else:
                        proposed = opt_rng.choice(hidden_indices, size=actual_batch_size, replace=False).tolist()
                ...
                if len(valid_indices) < actual_batch_size:
                    ...
                    extras = opt_rng.choice([idx for idx in hidden_indices if idx not in valid_indices], size=fill_needed, replace=False).tolist()
                    valid_indices.extend(extras)

                batch_history = []
                for i, next_idx in enumerate(valid_indices):
                    batch_history.append({
                        'step': steps_to_target, 'batch_position': i,
                        'selected_idx': int(next_idx), 'selected_value': float(y_pool[next_idx]),
                        'current_best': float(current_best_local),
                        'hidden_remaining': len(hidden_indices), 'found_target': found_target})
            ...
            return optimizer_name, {
                ...
                'final_best': float(np.max(y_observed)) if len(y_observed) > 0 else float('-inf'),
                'optimization_history': optimization_history,
                'surrogate_r2_history': surrogate_r2_history if optimizer_name == 'BO_GP_EI' else None,
                'elapsed_time': _time.monotonic() - _t0,
                'max_steps_capped': max_steps is not None and steps_to_target >= max_steps
            }

        # Run all optimizers in parallel using threading (they release the GIL during numpy/sklearn ops)
        from concurrent.futures import ThreadPoolExecutor

        # Derive a deterministic per-optimizer seed from the competition's local_rng
        opt_seeds = {name: int(local_rng.randint(0, 2**31)) for name in optimizers}

        with ThreadPoolExecutor(max_workers=len(optimizers)) as executor:
            futures = {
                executor.submit(
                    _run_one_optimizer, name, opt, competition_state, X_pool, y_pool,
                    target_idx, target_value, batch_size, opt_seeds[name], verbose,
                    max_steps=_optimizer_max_steps.get(name)
                ): name for name, opt in optimizers.items()
            }
            for future in futures:
                opt_name, result = future.result()
                optimizer_results[opt_name] = result
                if verbose:
                    logger.info(f"[Competition] {opt_name} completed: {result['steps_to_target']} steps, found_target={result['found_target']}, remaining_hidden={result.get('hidden_remaining', 'N/A')}")
    ...
    def run_competition_tournament(self, dataset_name: str,
                                  ...
                                  verbose: bool = True,
                                  competition_start: int = 0) -> Dict[str, Any]:
        """
        ...
        # competition_start allows skipping already-completed competitions (e.g. during top-up)
        synth_info_hash = int(hashlib.md5(f"synth_{synthetic_dataset_index}".encode()).hexdigest()[:8], 16) % 100000
        comp_indices = range(competition_start, competition_start + n_competitions)
        seeds = [
            self.random_state + synth_info_hash + i * 10000 + int(hashlib.md5(f"comp_{i}".encode()).hexdigest()[:8], 16) % 10000
            for i in comp_indices
        ]
        total = competition_start + n_competitions
        if n_jobs == 1:
            # Sequential execution
            competitions = []
            for idx, (i, seed) in enumerate(zip(comp_indices, seeds)):
                result = run_competition_with_seed(seed, competition_start + idx + 1, total)
                competitions.append(result)
        else:
            # Parallel execution
            competitions = Parallel(n_jobs=n_jobs, verbose=0)(
                delayed(run_competition_with_seed)(seed, competition_start + idx + 1, total)
