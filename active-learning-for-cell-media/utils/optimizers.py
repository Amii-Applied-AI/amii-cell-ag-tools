...
def _create_fast_kernel(n_features: int, complexity: str = "fast") -> Any:
    ...
    if complexity == "turbo":
        # Fixed-hyperparameter kernel — no optimizer overhead (~14ms vs ~700ms)
        return 1.0 * RBF(length_scale=1.0, length_scale_bounds="fixed") + WhiteKernel(0.1, noise_level_bounds="fixed")
...
class BOGPEIStrategy(OptimizationStrategy):
    ...
    def __init__(self, random_state: int = 0, debug: bool = False, fast_mode: bool = True):
        ...
        self._cached_gp = None
        self._cached_scaler = None
        self._last_refit_n = 0
        self._refit_interval = 10  # Refit GP every 10 new observations instead of every step
    ...
    def select_next_point(self, X_pool: np.ndarray, hidden_indices: List[int],
        ...
        # Reuse cached GP if we haven't added enough new points since last refit
        if (self._cached_gp is not None and
            n_obs - self._last_refit_n < self._refit_interval and
            n_obs > 5):
            gp, scaler = self._cached_gp, self._cached_scaler
        else:
            # Determine GP complexity based on data size and fast_mode
            if self.fast_mode:
                complexity = "turbo"
            else:
                complexity = "fast" if n_obs < 50 else "medium"
...
class SBOGPPVStrategy(OptimizationStrategy):
    ...
    def select_next_point(self, X_pool: np.ndarray, hidden_indices: List[int],
        ...
        n_obs = len(y_observed)
        # Reuse cached GP if not enough new points since last refit
        if (self._cached_gp is not None and
            n_obs - self._last_refit_n < self._refit_interval and n_obs > 5):
            gp, scaler = self._cached_gp, self._cached_scaler
            # Skip the GP fitting below
            X_pool = X_pool.astype(FAST_DTYPE)
            X_hidden = X_pool[hidden_indices]
            X_hidden_scaled = scaler.transform(X_hidden)
            try:
...
class SBOGPEITruncDEStrategy(OptimizationStrategy):
    ...
    def select_next_point(self, X_pool: np.ndarray, hidden_indices: List[int], 
        ...
        try:
            ...
            # Find the best candidate directly (DE refinement skipped —
            # we already evaluate EI on all discrete hidden points, so continuous
            # optimization + snap-to-nearest adds overhead with no benefit)
            best_local_idx = np.argmax(ei)
            selected_idx = hidden_indices[best_local_idx]
...
class SmartBOStrategy(OptimizationStrategy):
    ...
    def _train_ensemble_gps(self, X_observed_embedded: np.ndarray, y_observed: np.ndarray):
        """Train single GP model (ensemble reduced to 1 for speed)"""
        # Skip refit if data size hasn't changed (GP cache handles exact match,
        # but this avoids even the fingerprint + cache lookup overhead)
        n = len(y_observed)
        if hasattr(self, '_last_ensemble_n') and self._last_ensemble_n == n and len(self.ensemble_gps) > 0:
            return
        self._last_ensemble_n = n

        self.ensemble_gps = []
        try:
            data_fingerprint = _get_data_fingerprint(X_observed_embedded, y_observed)
            cache_key = f"{data_fingerprint}_ensemble_0"
            gp, scaler = _fit_fast_gp(
