...
const els = {
  ...
  surrogate: document.getElementById("surrogateSelect"),
...
function getAvailableOptimizers() {
  ...
  try {
    const surrogate = els.surrogate.value;
    const modeData = playgroundData[hiddenFrac]?.[difficulty]?.[raceType]?.[batch]?.[surrogate];
    if (!modeData) return [];
...
function updateConfigBadge() {
  ...
  const datasetLabel = {
    ...
    DBO_rat_myocyte: "DBO Rat Myocyte",
    MOBO_rat_myocyte: "MOBO Rat Myocyte",
  ...
  const surrogateName = {
    GP: "Gaussian process", RF: "Random forest",
    NN: "Neural network", BNN: "Bayesian neural network",
  }[els.surrogate.value] || els.surrogate.value;
  const surrogateDesc = difficulty === "Hard"
    ? `${surrogateName} landscape with injected noise (Hard mode)`
    : `${surrogateName} landscape, low noise (Regular mode)`;
  ...
  const setupDesc = isHide
    ...
    : "S = 200 evaluations per competition; R = 10 initial points";
...
function hasDataForSelection() {
  ...
  try {
    const surrogate = els.surrogate.value;
    const data = playgroundData[hiddenFrac]?.[difficulty]?.[raceType]?.[batch]?.[surrogate];
...
function generateBarData(selected) {
  ...
  try {
    const surrogate = els.surrogate.value;
    const modeData = playgroundData[hiddenFrac][difficulty]["Hide_The_Label"][batch]?.[surrogate];
    if (!modeData) {
      const seedBase = stringHash(`hide:${selected.join(",")}:${batch}:${hiddenFrac}`);
      els.note.textContent = "Showing placeholder data — no results available for this configuration";
      return generatePlaceholderBarData(selected, seedBase);
...
function generateLineData(selected) {
  ...
  try {
    const surrogate = els.surrogate.value;
    const modeData = playgroundData[hiddenFrac][difficulty]["Open_Race"][batch]?.[surrogate];
    if (!modeData) {
      const seedBase = stringHash(`open:${selected.join(",")}:${batch}:${hiddenFrac}`);
      els.note.textContent = "Showing placeholder data — no results available for this configuration";
      return generatePlaceholderLineData(selected, seedBase);
...
async function init() {
  ...
  els.surrogate.addEventListener("change", onParameterChange);
